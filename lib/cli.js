...
cli.responders.help = () => {
    ...
    const commands = {
        ...
        'list logs': 'Show a list of all the logs available to be read (compressed only)',
...
// List logs directly from the shell
cli.responders.listLogs = () => {
    ...
    ls.stdout.on('data', (dataObj) => {
        ...
        cli.verticalSpace()
        logFileNames.forEach(logFileName => {
            if (typeof(logFileName) == 'string' && logFileNames.length > 0 && logFileName.includes('-')) {
        ...
        _logs.decompress(logFileName, (err, strData) => {
            if (!err && strData) {
                ...
                arr.forEach(jsonString => {
                    const logObj = helpers.parseJsonToObject(jsonString)
