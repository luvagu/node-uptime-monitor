...
workers.performCheck = (originalCheckData) => {
    ...
    const requestDetails = {
        ...
        // },
        // We need to specify a host wich is the same hostname as the requested URL
        host: hostName,
        // In some cases we may need to include a port
...
workers.rotateLogs = () => {
    ...
    _logs.list(false, (err, logs) => {
        if (!err && logs && logs.length > 0) {
            ...
            console.log('Error: could not find any log files to rotate');
        }
    })
}

// Timer to execute the logging-rotation once per day
...
workers.init = () => {
    ...
    // Compress all the losgs immediately
    workers.rotateLogs()
