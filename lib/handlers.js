...
const obs = new PerformanceObserver((list, observer) => {
    list.getEntries().forEach(measeurement => {
        debug('\x1b[33m%s\x1b[0m', `${measeurement.name} ${measeurement.duration}`)
    })
    //observer.disconnect()
  })
...
handlers.index = (data, callback) => {
    ...
    if (data.method == 'get') {
        ...
        const templateData = {
            'head.title': 'Site Uptime Monitoring - Made Simple',
...
handlers.sessionDeleted = (data, callback) => {
    ...
    if (data.method == 'get') {
        ...
        const templateData = {
            ...
            'head.description': 'You have been signed out of your account',
...
handlers._tokens.post = (data, callback) => {
    _performance.mark('entered the function')
    ...
    if (phone && password) {
        ...
        _data.read('users', phone, (err, userData) => {
            ...
            if (!err && userData) {
                ...
                _performance.mark('beginning password hasing')
                ...
                if (hashedPassword == userData.hashedPassword) {
                    ...
                    _data.create('tokens', tokenId, tokenObject, (err) => {
                        ...
                        // Gather all the measurments
                        _performance.measure('Beginning to end', 'entered the function', 'storing token completed')
                        _performance.measure('Validating user input', 'entered the function', 'inputs validated')
                        _performance.measure('User lookup', 'beginning user lookup', 'user lookup completed')
                        _performance.measure('Password hashing', 'beginning password hasing', 'password hashing completed')
                        ...
                        // measurements.forEach(measeurement => {
                        //     debug('\x1b[33m%s\x1b[0m', `${measeurement.name} ${measeurement.duration}`)
                        // })
